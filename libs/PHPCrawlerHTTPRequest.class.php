...
class PHPCrawlerHTTPRequest {
    ...
    protected function readResponseContent($stream_to_file = false, &$error_code, &$error_string, &$document_received_completely) {
        ...
        while ($document_completed == false) {
            ...
            if (($gzip_encoded_content == false && $stream_to_file == false && strlen($source_portion) >= $this->content_buffer_size && !$this->LinkFinder->dom_excludes && !$this->LinkFinder->dom_constraints) || $document_completed == true) {
                if (PHPCrawlerUtils::checkStringAgainstRegexArray($this->lastResponseHeader->content_type, $this->linksearch_content_types)) {
                    ...
                    if ($document_completed) {
                        $html = str_get_html($source_portion);
                        if (!empty($this->LinkFinder->dom_excludes)) {
                            // exclude certain nodes from the DOM tree
                            foreach ($this->LinkFinder->dom_excludes as $exclude) {
                                if ($exclude[1] === null) {
                                    foreach ($html->find($exclude[0], $exclude[1]) as $elem) {
                                            $elem->outertext = '';
                                    }
                                } else {
                                    $html->find($exclude[0], $exclude[1])->outertext = '';
                                }
                            }
                            $html->load($html->save());
                        }
                        if (!empty($this->LinkFinder->dom_constraints)) {
                            // only search certain DOM nodes for links
                            foreach ($this->LinkFinder->dom_constraints as $constraint) {
                                if ($constraint[1] === null) {
                                    foreach ($html->find($constraint[0], $constraint[1]) as $elem) {
                                        $this->LinkFinder->findLinksInHTMLChunk($elem->outertext);
                                    }
                                } else {
                                    $this->LinkFinder->findLinksInHTMLChunk($html->find($constraint[0], $constraint[1])->outertext);
                                }
                            }
                        } else {
                            $this->LinkFinder->findLinksInHTMLChunk($html);
                        }
